...
as$de=>$W)$H.="<tr>".($X!=array_values($X)?"<th>".h($de):"")."<td>".select_value($W,$_,$o,$di);return"<table cellspacing='0'>$H</table>";}if(!$_)$_=$b->selectLink($X,$o);if($_===null){if(is_mail($X))$_="$X";if(is_url($X))$_=$X;}$H=$b->editVal($X,$o);if($H!==null){if(!is_utf8($H))$H="\0";elseif($di!=""&&is_shortable($o))$H=shorten_utf8($H,max(0,+$di));else$H=h($H);}return$b->selectVal($H,$_,$o,$X);}function
