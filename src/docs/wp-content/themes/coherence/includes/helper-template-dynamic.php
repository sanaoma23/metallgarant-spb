...
*/
	...
	function hr($element)
	{
		...
		switch($element['saved_value'][0]['dynamic_hr'])
		{
			...
			case 'split': 			$output .= "</div>";
									...
									$output .= "</div>  <div class='container container_split $extraClass'>";
	...
	*/
	...
	{	
		...
		if($heading)
		{
			$output = avia_title($heading, $subtitle, false, "dynamic_element dynamic_el_".$this->dynamic_counter);
		}
	...
	function textarea($element)
	{
		...
		switch($dynamic_text_styling)
		{
			...
			case 'callout': 	$output .= "<div class='outer_callout $extraClass'>";
								...
								{
									$output .= "<a class='big_button avia-button button b2' href='".avia_get_link($element['saved_value'][0], 'dynamic_text_button_')."'>".$dynamic_text_button."</a>";
		...
		$output = str_replace('/modulnyie-zdaniya', '/modulnyie-zdaniya/', $output);
	...
	*/
	...
	function post_page($element)
	{
		...
		if($additional_loop->have_posts())
		{
			...
			$output .= "</div>
							           <div class='zv speczv'><span class='quest'>Остались вопросы? <a href='#contact_form_pop' class='order-call fancybox-inline specknop' style='position: static;'>Заказать звонок</a></span>
