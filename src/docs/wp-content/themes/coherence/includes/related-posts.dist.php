...
if($is_portfolio) // its a portfolio post
{
	...
	$post_class 		= "one_fifth no_margin";
	$format 			= 'portfolio';
     ...
     if($tag_ids) 
     {
		...
		{     	
  		...
  		{ 
     		...
     		$output .= "<h5 class='related_title'>".__('Также ознакомьтесь:', 'avia_framework')."</h5>";
     		...
     		{	
	 			...
	 			if($is_portfolio) { $subtitle = avia_post_meta($post->ID, 'subtitle'); if(!$subtitle) $subtitle = ""; }
	 			...
	 			$output .= $image;
	 			$output .= "</span>\n";
	 			$output .= avia_title(avia_backend_truncate(get_the_title(), 300, " "), $subtitle, 'portfolio-title');
	 			$output .= "\n</a>";
