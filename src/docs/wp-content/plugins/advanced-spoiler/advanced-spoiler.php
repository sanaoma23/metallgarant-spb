...
/*		while(preg_match($this->regex, $content)) {
	...
	function _replace($m) {
		...
		if ($this->op['feed_visibility'] == 'hide' && (is_feed() || $doing_rss)) {
			return "<p class='ferst' style='text-align: left'><a href='$href' title='".__('Visit blog to check out this spoiler', $this->domain)."'>[[".__('Visit blog to check out this spoiler', $this->domain)."]]</a></p>";
		}
		...
		if ($this->op['plugin_mode'] == 'simple') {
			return "<p class='ferst' style='text-align: left'><a onfocus='this.blur();' class='spoiler-tgl collapsed' href='$href' id='{$id}_tgl' onclick=\"s_toggleDisplay(document.getElementById('{$id}'), this, '".attribute_escape($show)."', '".attribute_escape($hide)."'); return false;\">$show</a></p>\n\n<div id='$id' class='spoiler-body' style='display:none;'>\n\n".trim($text)."</div>";
		}
		...
		return "<p class='ferst' style='text-align: left'><a class='spoiler-tgl collapsed' href='$href' id='{$id}_tgl' rev='$rev'>$show</a></p>\n\n<div id='$id' class='spoiler-body'>\n\n".trim($text)."</div>";
	}
