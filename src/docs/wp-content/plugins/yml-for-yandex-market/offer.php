		...
		*/
			...
			if (in_array($CurCategoryId, $no_group_id_arr)) {
			 ...
			 foreach ($attributes as $param) {					
				...
				if ($separator_type === 'type4') {
					$param_at_name .= $sd.ucfirst(urldecode($param_val)).' ';
				} else {
		...
		if (!empty($country_of_origin) && $country_of_origin !== 'off') {
			...
			if (!empty($country_of_origin_yml)) {	
				$result_yml_country_of_origin = "<country_of_origin>".ucfirst(urldecode($country_of_origin_yml))."</country_of_origin>".PHP_EOL;		
			} else {
		...
		if (!empty($yfym_manufacturer) && $yfym_manufacturer !== 'disabled') {
			...
			if (!empty($yfym_manufacturer_yml)) {	
				$result_yml_manufacturer = "<manufacturer>".ucfirst(urldecode($yfym_manufacturer_yml))."</manufacturer>".PHP_EOL;		
			} else {
		...
		if (class_exists('Perfect_Woocommerce_Brands') && $vendor === 'sfpwb') {
		 ...
		 if ($vendor !== 'disabled') {
			...
			if (!empty($vendor_yml)) {
				$result_yml_vendor = '<vendor>'.ucfirst(urldecode($vendor_yml)).'</vendor>'.PHP_EOL;
			} else {
		...
		switch ($yfym_shop_sku) { /* disabled, sku, products_id, или id */
			...
			default:
				...
				if (!empty($yfym_shop_sku_yml)) {
					...
					$result_yml_shop_sku = '<shop-sku>'.ucfirst(urldecode($yfym_shop_sku_yml)).'</shop-sku>'.PHP_EOL;
				} else {
